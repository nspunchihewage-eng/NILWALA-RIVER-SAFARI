...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupStatsRealtime() {
        ...
        // Real-time Today's Visitors
        const now = new Date();
        const mm_str = String(now.getMonth() + 1).padStart(2, '0');
        const dd_str = String(now.getDate()).padStart(2, '0');
        const todayISO = `${now.getFullYear()}-${mm_str}-${dd_str}`;
        
        // Ensure the card exists in the DOM (in case of old HTML cache)
        if (!document.getElementById('stat-visitors')) {
            const grid = document.querySelector('.stats-grid-4');
            if (grid) {
                const newCard = document.createElement('div');
                newCard.className = 'glass-panel stat-card text-center';
                newCard.style.cssText = 'padding: 2.5rem 2rem; border-radius: 20px; background: linear-gradient(145deg, rgba(20,20,22,0.8), rgba(15,15,18,0.9)); border: 1px solid rgba(255,255,255,0.05); position: relative; overflow: hidden;';
                newCard.innerHTML = `
                    <div style="position: absolute; top: -15px; right: -15px; font-size: 6rem; opacity: 0.05; color: #e74c3c;"><i class="fas fa-users-viewfinder"></i></div>
                    <i class="fas fa-users-viewfinder" style="font-size: 2.5rem; color: #e74c3c; margin-bottom: 1rem; display: block; position: relative; z-index: 1;"></i>
                    <h3 id="stat-visitors" style="font-size: 3rem; color: white; margin-bottom: 0; line-height: 1;">0</h3>
                    <p style="color: var(--text-muted); text-transform: uppercase; font-size: 0.85rem; letter-spacing: 1px; margin-top: 10px;">Visitors Today</p>
                `;
                grid.appendChild(newCard);
            }
        }

        db.collection("analytics").doc(todayISO).onSnapshot(doc => {
            const el = document.getElementById('stat-visitors');
            if (el && doc.exists) {
                el.innerText = doc.data().hits || 0;
            } else if (el) {
                el.innerText = '0';
            }
        }, err => console.log("Visitors Stat error:", err));
    ...
    function closePackageForm() {
        ...
        const usdPrev = document.getElementById('usd-preview');
        if (usdPrev) usdPrev.innerText = '$0';
    }

    // Auto-calculate USD preview in management portal
    const packPriceInput = document.getElementById('pack-price');
    const usdPreview = document.getElementById('usd-preview');
    if (packPriceInput && usdPreview) {
        packPriceInput.addEventListener('input', (e) => {
            const lkr = parseFloat(e.target.value) || 0;
            usdPreview.innerText = `$${(lkr / EXCHANGE_RATE).toFixed(0)}`;
        });
    }
    ...
    function renderAnalyticsChart() {
        ...
        for (let i = daysToLookBack - 1; i >= 0; i--) {
            ...
            // Construct multiple possible formats for robustness
            ...
            let dailyVisitors = allAnalytics[localFormat] || allAnalytics[isoFormat] || 0;
    ...
    // Add Manual Sync Tools to verify the fix
    if (!document.getElementById('btn-refresh-stats')) {
        const header = document.querySelector('.admin-header-flex');
        if (header) {
            ...
            btnRef.id = 'btn-refresh-stats';
            btnRef.className = 'btn-secondary-light';
            btnRef.innerHTML = '<i class="fas fa-sync"></i> Refresh Graph';
            btnRef.onclick = () => {
                btnRef.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
                renderAnalyticsChart();
                setTimeout(() => btnRef.innerHTML = '<i class="fas fa-sync"></i> Refresh Graph', 600);
            };
            
            const btnTest = document.createElement('button');
            btnTest.className = 'btn-primary';
            btnTest.style.background = '#27ae60';
            btnTest.style.borderColor = '#27ae60';
            btnTest.innerHTML = '<i class="fas fa-check-double"></i> Verify Sync';
            btnTest.onclick = async () => {
                const originalText = btnTest.innerHTML;
                btnTest.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Syncing...';
                try {
                    const d = new Date();
                    const iso = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0') + '-' + String(d.getDate()).padStart(2, '0');
                    await db.collection("analytics").doc(iso).set({
                        hits: firebase.firestore.FieldValue.increment(1),
                        lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
                    }, { merge: true });
                    alert("Sync Verified! You just manually tracked a visit. Today's counter will increase.");
                } catch (e) {
    ...
    // Removal of redundant calls that were running before auth was established
    // (Consolidated inside onAuthStateChanged instead)
    ...
    function setupSocialListener() {
        db.collection("settings").doc("socials").onSnapshot((doc) => {
            if (doc.exists) {
                const data = doc.data();
                document.getElementById('social-fb').value = data.facebook || '';
                document.getElementById('social-ig').value = data.instagram || '';
                document.getElementById('social-tt').value = data.tiktok || '';
                document.getElementById('social-ta').value = data.tripadvisor || '';
            }
        });
    }
    ...
    function setupSocialForm() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            const confirm = document.getElementById('social-save-confirm');
            ...
            try {
                await db.collection("settings").doc("socials").set({
                    facebook: document.getElementById('social-fb').value.trim(),
                    instagram: document.getElementById('social-ig').value.trim(),
                    tiktok: document.getElementById('social-tt').value.trim(),
                    tripadvisor: document.getElementById('social-ta').value.trim(),
    ...
    // --- 10. MAP LOCATION LOGIC ---
    function setupMapListener() {
        const mapInput = document.getElementById('map-url');
        const mapPreview = document.getElementById('admin-map-preview');
        
        if (!mapInput) return;

        db.collection("settings").doc("location").onSnapshot((doc) => {
            if (doc.exists) {
                const data = doc.data();
                ...
                if (url && url.startsWith('http')) {
                    mapPreview.innerHTML = `<iframe src="${url}" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy"></iframe>`;
                } else {
                    mapPreview.innerHTML = `<span style="color: var(--text-muted);">No Map URL set yet.</span>`;
                }
            }
        });
    }
    ...
    function setupMapForm() {
        const form = document.getElementById('form-map-location');
        if (!form) return;

        form.addEventListener('submit', async (e) => {
            e.preventDefault();
            const btn = form.querySelector('button[type="submit"]');
            const confirm = document.getElementById('map-save-confirm');
            
            const originalText = btn.innerHTML;
            btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Saving Map...';
            btn.disabled = true;

            try {
                const url = document.getElementById('map-url').value.trim();
                await db.collection("settings").doc("location").set({
                    mapUrl: url,
                    updatedAt: new Date().toISOString()
                });
                ...
                alert("Failed to update map location.");
            } finally {
                btn.innerHTML = originalText;
                btn.disabled = false;
            }
