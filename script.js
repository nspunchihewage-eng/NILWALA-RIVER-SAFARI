...
// Initial trigger on DOMContentLoaded for perceived speed
document.addEventListener('DOMContentLoaded', () => {
    // Show preloader for a minimum period for cinematic effect, but not too long
    setTimeout(hidePreloader, 2000); 
    
    // Everything else can init on DOMContentLoaded or Load
});

// Fallback on full window load
...
function trackVisit() {
    ...
    if (window._visitorTracked) return;

    // Use window.db explicitly - this is our Firestore instance
    const firestore = window.db || (typeof db !== 'undefined' ? db : null);
    
    if (!firestore) {
        setTimeout(trackVisit, 600); 
        return;
    }
    ...
    try {
        ...
        const docRef = firestore.collection("analytics").doc(dateISO);
        
        // Prevent re-firing
        window._visitorTracked = true;

        docRef.set({
            hits: firebase.firestore.FieldValue.increment(1),
            lastUpdated: firebase.firestore.FieldValue.serverTimestamp()
        }, { merge: true })
        .then(() => console.log("Visit +1:", dateISO))
        .catch(err => {
            console.warn("Retrying analytics...");
            docRef.get().then(snap => {
                if (!snap.exists) {
                    docRef.set({ hits: 1, lastUpdated: new Date().toISOString() });
                } else {
                    docRef.update({ hits: (snap.data().hits || 0) + 1 });
                }
            });
        });
    } catch (e) {
...
// Currency Configuration (1 USD = 300 LKR as average rate)
const EXCHANGE_RATE = 300;
let currentCurrency = localStorage.getItem('selected_currency') || 'USD';

// Observe all animatable elements
document.addEventListener('DOMContentLoaded', () => {
    // Visit tracking is handled by trackVisit() on window load (line 53)
    // to ensure Firebase is fully initialized and avoid double-counting.
    ...
    const currBtns = document.querySelectorAll('.currency-btn');
    const currSwitcher = document.querySelector('.currency-switcher');
    ...
    const updateAllPrices = (currency) => {
        ...
        // Update Button UI
        if (currSwitcher) currSwitcher.setAttribute('data-curr', currency);
        currBtns.forEach(btn => {
            if (btn.getAttribute('data-curr') === currency) {
        ...
        // Update all price elements with data-lkr
        document.querySelectorAll('.price[data-lkr]').forEach(el => {
            const lkr = parseFloat(el.getAttribute('data-lkr'));
            if (currency === 'USD') {
                el.innerHTML = `$${(lkr / EXCHANGE_RATE).toFixed(0)}<span>/person</span>`;
            } else {
                el.innerHTML = `Rs. ${lkr.toLocaleString()}<span>/person</span>`;
            }
        });
    };
    ...
    currBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            updateAllPrices(btn.getAttribute('data-curr'));
        });
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (galleryContainer) {
        db.collection("gallery").orderBy("createdAt", "desc").limit(2).onSnapshot((snapshot) => {
            ...
            snapshot.forEach((doc, index) => {
                ...
                div.innerHTML = `
                    <img loading="lazy" src="${data.url}" alt="${escapeHTML(data.title)}">
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', async (e) => {
            ...
            // Capture full international number from intl-tel-input
            let phoneValue = document.getElementById('book-phone').value.trim();
            if (window.itiBooking) {
                phoneValue = window.itiBooking.getNumber();
            }
            const phone = stripHTML(phoneValue);
    ...
    const translations = {
        en: {
            ...
            // Services
            serv_badge: "Premium Offerings", serv_title: "Our Services", serv_subtitle_home: "Discover our range of premium river experiences, specifically designed to showcase the best of Nilwala's natural wonders.", serv_safari: "Boat Safari in Nilwala River", serv_watch: "Croc Watching in Nilwala River", serv_bird: "Bird Watching in Nilwala River", serv_fish: "Fishing in Nilwala River", serv_monkey: "Monkey Feeding in Nilwala River", serv_transfer: "Free Pick & Drop",
            guides_badge: "Expert Team", guides_title: 'Meet Your <span class="highlight">Guides</span>', guides_desc: "Experience the expertise of Matara's most trusted river navigators, dedicated to your safety and adventure.",
            ...
            monk_title: "Monkey Feeding in Nilwala River", monk_subtitle: "Interactive Wildlife", monk_desc: "Get up close and personal with the playful native macaque monkeys that inhabit the lush mangrove forests along the Nilwala riverbanks.",
            ...
            fish_badge: "Traditional & Sport Fishing", fish_title: 'Fishing in <span class="highlight">Nilwala River</span>', fish_desc: "Experience the thrill of local river fishing or explore deep-sea excursions. We provide premium gear and expert guidance for all skill levels.",
            fish_feat_1: "<strong>All Equipment Included:</strong> We provide professional rods, reels, and bait so you can focus on the catch.",
            fish_feat_2: "<strong>River & Sea Options:</strong> Enjoy calm river fishing or ask about our seasonal deep-sea fishing trips.",
            fish_feat_3: "<strong>Guided & Safe:</strong> Learn traditional Sri Lankan techniques with our experienced local fishermen.",
            ...
            form_name: "Your Name", form_phone: "Phone / WhatsApp", form_date: "Select Date", form_guests: "No. of Guests", form_special: "Any special requests? (Optional)", form_submit: "Submit Booking Request", sel_package: "Select Package", other: "Other / Not Sure",
            // Footer
        ...
        si: {
            ...
            serv_badge: "අපගේ සේවාවන්", serv_title: "අපගේ සේවාවන්", serv_subtitle_home: "නිල්වලා ගඟේ ස්වාභාවික සෞන්දර්යය මැනවින් විඳගත හැකි පරිදි සකස් කරන ලද අපගේ සුවිශේෂී සේවාවන් අත්විඳින්න.", serv_safari: "නිල්වලා ගඟේ බෝට්ටු සෆාරි (Boat Safari in Nilwala River)", serv_watch: "නිල්වලා ගඟේ කිඹුලන් නැරඹීම (Croc Watching)", serv_bird: "නිල්වලා ගඟේ කුරුල්ලන් නැරඹීම (Bird Watching)", serv_fish: "නිල්වලා ගඟේ මසුන් ඇල්ලීම (Fishing in Nilwala River)", serv_monkey: "නිල්වලා ගඟේ වඳුරන්ට කෑම දීම (Monkey Feeding)", serv_transfer: "නවාතැන්පොළේ සිට ගෙන ඒම සහ රැගෙන යාම",
            guides_badge: "පළපුරුදු කණ්ඩායම", guides_title: '<span class="highlight">මඟපෙන්වන්නන්</span> හමුවන්න', guides_desc: "ඔබේ ආරක්ෂාව සහ විනෝදය වෙනුවෙන් කැප වූ මාතර වඩාත්ම විශ්වාසවන්ත ගංගා සංචාරක මඟපෙන්වන්නන්ගේ අත්දැකීම් විඳගන්න.",
            ...
            monk_title: "නිල්වලා ගඟේ වඳුරන්ට ආහාර ලබා දීම (Monkey Feeding in Nilwala River)", monk_subtitle: "සජීවී වනජීවී", monk_desc: "නිල්වලා ගඟ ඉවුර දිගේ සශ්‍රීක කඩොලාන වනාන්තරවල වාසය කරන සෙල්ලක්කාර දේශීය වඳුරන් සමඟ සමීප අත්දැකීමක් ලබා ගන්න.",
            ...
            fish_badge: "සම්ප්‍රදායික සහ ක්‍රීඩා මසුන් ඇල්ලීම", fish_title: 'නිල්වලා ගඟේ <span class="highlight">මසුන් ඇල්ලීම</span> (Fishing in Nilwala River)', fish_desc: "දේශීය ගංගා මසුන් ඇල්ලීමේ හෝ ගැඹුරු මුහුදේ මසුන් ඇල්ලීමේ අත්දැකීමක් ලබා ගන්න. අපි ඔබ වෙනුවෙන් ඉහළ මට්ටමේ උපකරණ සහ මඟපෙන්වීම් සපයන්නෙමු.",
            fish_feat_1: "<strong>සියලුම උපකරණ අපෙන්:</strong> මසුන් ඇල්ලීමට අවශ්‍ය සියලුම නවීන සහ වෘත්තීය මට්ටමේ උපකරණ අපි ඔබ වෙනුවෙන් සපයන්නෙමු.",
            fish_feat_2: "<strong>ගංගා සහ මුහුදු විකල්ප:</strong> ඔබට අවශ්‍ය නම් ගංගාවේ මෙන්ම ගැඹුරු මුහුදේ සංචාර සඳහාද අපගේ සේවාව ලබාගත හැක.",
            fish_feat_3: "<strong>ආරක්ෂිත සහ මඟපෙන්වීම් සහිත:</strong> පළපුරුදු මඟපෙන්වන්නන් සමඟින් සම්ප්‍රදායික ශ්‍රී ලංකා මසුන් ඇල්ලීමේ ක්‍රමවේද ආරක්ෂිතව ඉගෙන ගන්න.",
            ...
            form_name: "ඔබේ නම", form_phone: "දුරකථන / WhatsApp අංකය", form_date: "දිනය තෝරන්න", form_guests: "සංචාරකයින් සංඛ්‍යාව", form_special: "විශේෂ ඉල්ලීම් (අත්‍යවශ්‍ය නොවේ)", form_submit: "වෙන්කිරීම සඳහා යොමුකරන්න", sel_package: "පැකේජය තෝරන්න", other: "වෙනත් / නිශ්චිත නැත",
        ...
        de: {
            ...
            serv_badge: "Premium-Angebote", serv_title: "Unsere Leistungen", serv_safari: "Bootssafari im Nilwala-Fluss", serv_watch: "Krokodilbeobachtung im Nilwala-Fluss", serv_bird: "Vogelbeobachtung im Nilwala-Fluss", serv_fish: "Angeln im Nilwala-Fluss", serv_monkey: "Affenfütterung im Nilwala-Fluss", serv_transfer: "Gratis Abhol- & Bringservice",
            ...
            monk_title: "Affenfütterung im Nilwala-Fluss (Monkey Feeding in Nilwala River)", monk_subtitle: "Interaktive Wildtiere", monk_desc: "Erleben Sie die verspielten einheimischen Makaken hautnah, die in den üppigen Mangrovenwäldern entlang der Nilwala-Ufer leben.",
            ...
            form_name: "Ihr Name", form_phone: "Telefon / WhatsApp", form_date: "Datum wählen", form_guests: "Anzahl der Gäste", form_special: "Besondere Wünsche? (Optional)", form_submit: "Buchungsanfrage senden", sel_package: "Paket auswählen", other: "Andere / Unsicher",
            ...
            river_good: "Gut", river_normal: "Normal", river_caution: "Vorsicht", river_alert: "Alarm", river_label: "Flusszustand", change_language: "Sprache Ändern", serv_view_more: "Alle Leistungen ansehen"
        },
        ru: {
            ...
            serv_badge: "Премиум услуги", serv_title: "Наши услуги", serv_safari: "Сафари на лодке по реке Нилвала", serv_watch: "Наблюдение за крокодилами на реке Нилвала", serv_bird: "Наблюдение за птицами на реке Нилвала", serv_fish: "Рыбалка на реке Нилвала", serv_monkey: "Кормление обезьян на реке Нилвала", serv_transfer: "Бесплатный трансфер",
            ...
            monk_title: "Кормление обезьян на реке Нилвала (Monkey Feeding in Nilwala River)", monk_subtitle: "Интерактивная природа", monk_desc: "Познакомьтесь поближе с игривыми местными макаками, обитающими в пышных мангровых лесах вдоль берегов реки Нилвала.",
            ...
            form_name: "Ваше имя", form_phone: "Телефон / WhatsApp", form_date: "Выберите дату", form_guests: "Количество гостей", form_special: "Особые пожелания? (Необязательно)", form_submit: "Отправить запрос на бронирование", sel_package: "Выбрать пакет", other: "Другое / Не уверен",
        ...
        fr: {
            ...
            serv_badge: "Offres Premium", serv_title: "Nos Services", serv_safari: "Safari en bateau sur la rivière Nilwala", serv_watch: "Observation de crocodiles sur la rivière Nilwala", serv_bird: "Observation d'oiseaux sur la rivière Nilwala", serv_fish: "Pêche sur la rivière Nilwala", serv_monkey: "Nourrissage de singes sur la rivière Nilwala", serv_transfer: "Transfert gratuit",
            ...
            monk_title: "Nourrissage des singes sur la rivière Nilwala (Monkey Feeding in Nilwala River)", monk_subtitle: "Vie sauvage interactive", monk_desc: "Approchez-vous des macaques indigènes joueurs qui habitent les forêts de mangroves luxuriantes le long des rives du fleuve Nilwala.",
            ...
            form_name: "Votre nom", form_phone: "Téléphone / WhatsApp", form_date: "Choisir la date", form_guests: "Nombre d'invités", form_special: "Demandes spéciales ? (Facultatif)", form_submit: "Envoyer ma demande de réservation", sel_package: "Sélectionner un forfait", other: "Autre / Incertain",
    ...
    const setLanguage = (lang) => {
        ...
        document.querySelectorAll('[data-t]').forEach(el => {
            ...
            if (dict[key]) {
                ...
                if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
                    ...
                    const htmlKeys = ['refresh_1', 'refresh_2', 'refresh_3', 'refresh_4', 'monk_feat_1', 'monk_feat_2', 'monk_feat_3', 'monk_title', 'fish_title'];
                    ...
                    if (htmlKeys.includes(key) || el.classList.contains('highlight') || el.classList.contains('highlight-italic') || el.querySelector('.highlight') || el.querySelector('.highlight-italic')) {
                        ...
                        if (lang === 'si') {
                            ...
                            if (key === 'hospitality_title') {
                                ...
                                if (translated.includes("Nilwala River")) {
                                    translated = translated.replace("Nilwala River", '<span class="highlight">Nilwala River</span>');
                                }
                            }
        ...
        if (langDisplay) {
            langDisplay.innerHTML = `<img loading="lazy" src="https://flagcdn.com/w20/${currentFlag}.png" alt="${lang}"> ${currentText} <i class="fas fa-chevron-down"></i>`;
        }
    ...
    // Initialize International Phone Number Input (intl-tel-input)
    const phoneInput = document.querySelector("#book-phone");
        ...
        const iti = window.intlTelInput(phoneInput, {
            ...
            geoIpLookup: callback => {
                fetch("https://ipapi.co/json")
                    ...
                    .catch(() => callback("lk")); // Default to Sri Lanka if IP lookup fails
            },
            separateDialCode: true,
            utilsScript: "https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/19.2.16/js/utils.js"
        });

        // Store it globally if needed for validation later
        window.itiBooking = iti;
        
        // Ensure the input field padding handles the dial code
        phoneInput.parentElement.style.width = "100%";
    }

    // Auto-scroll horizontal grids on mobile for UX touch sense
    const horizontalGrids = document.querySelectorAll('.packages-grid, .services-grid, .guides-grid, .testimonials-grid, #dynamic-gallery');
    horizontalGrids.forEach(grid => {
        let scrollInterval;
        let isHoveredOrTouched = false;

        const doAutoScroll = () => {
            if (window.innerWidth > 992 || isHoveredOrTouched) return;
        ...
        const startAutoScroll = () => { 
            ...
            scrollInterval = setInterval(doAutoScroll, 3000); 
        };
        ...
        // Pause interaction
        const pauseEvents = ['touchstart', 'mouseenter'];
        const resumeEvents = ['touchend', 'mouseleave'];

        pauseEvents.forEach(e => grid.addEventListener(e, () => {
            isHoveredOrTouched = true;
            stopAutoScroll();
        }));

        resumeEvents.forEach(e => grid.addEventListener(e, () => {
...
// --- Social Media Real-time Sync ---
function setupRealtimeSocialLinks() {
    const firestore = window.db || (typeof db !== 'undefined' ? db : null);
    if (!firestore) return;

    firestore.collection("settings").doc("socials").onSnapshot((doc) => {
        if (doc.exists) {
            const data = doc.data();
            
            // Map icons to their links
            const linksMap = {
                '.fa-facebook-f': data.facebook,
                '.fa-instagram': data.instagram,
                '.fa-tiktok': data.tiktok,
                '.fa-tripadvisor': data.tripadvisor
            };

            Object.entries(linksMap).forEach(([iconClass, url]) => {
                const icons = document.querySelectorAll(iconClass);
                icons.forEach(icon => {
                    const link = icon.closest('a');
                    if (link && url && url.startsWith('http')) {
                        link.href = url;
                        link.target = "_blank";
                    }
                });
            // --- Google Map Real-time Sync ---
function setupRealtimeMap() {
    const firestore = window.db || (typeof db !== 'undefined' ? db : null);
    const mapIframe = document.getElementById('footer-map');
    if (!firestore || !mapIframe) return;

    firestore.collection("settings").doc("location").onSnapshot((doc) => {
        if (doc.exists) {
            const data = doc.data();
            const url = data.mapUrl || '';
            if (url && url.startsWith('http')) {
                mapIframe.src = url;
                mapIframe.closest('.map-container').style.display = 'block';
            } else {
                mapIframe.closest('.map-container').style.display = 'none';
            }
        }
    });
}
